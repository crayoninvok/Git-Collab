...
export default function ResetPasswordPromotor() {
  ...
  const token = Array.isArray(params?.token) ? params.token[0] : params.token; // Safely extract token
    ...
    if (token) {
      ...
      if (savedToken) {
        ...
        toast.error("No reset token found. Redirecting to login...", {
          position: "top-right",
          autoClose: 3000,
          onClose: () => router.push("/login/loginpromotor"),
        });
      }
    }
  }, [token, router]);
  ...
  const handleResetPassword = async (values: { newPassword: string; confirmPassword: string }) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL_BE}/auth/resetPasswordPromotor`, {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
      ...
      <div className="bg-gradient-to-br from-gray-800/90 to-black/70 backdrop-blur-md border border-gray-700 rounded-3xl p-8 lg:p-12 shadow-xl w-full max-w-[400px]">
        ...
        <p className="mt-6 text-center text-sm text-gray-400">
          ...
          <a href="/login/loginpromotor" className="text-blue-400">
            Login here
          </a>
        </p>
      </div>
